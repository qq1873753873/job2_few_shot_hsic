...
class LDA_SLIC(object):
    ...
    def PCA_Process(self, curr_labels):
        '''
        :param curr_labels: height * width
        :return:
        '''
        ...
        idx = np.where(curr_labels != 0)[0]
        x = self.x_flatt[idx]
        pca = PCA(n_components=int(max(curr_labels)-1))
        # pca = PCA(n_components=25)
        pca.fit(x)
        X_new = pca.transform(self.x_flatt)
        print('贡献率：', pca.explained_variance_ratio_)
        return np.reshape(X_new, [self.height, self.width, -1])

    def SLIC_Process(self, img, scale=25):
        n_segments_init = self.height * self.width / scale
        myslic = SLIC(img, n_segments=n_segments_init, labels=self.labels, compactness=1, sigma=1, min_size_factor=0.1,
                      max_size_factor=2)
    ...
    def simple_superpixel(self, scale):
        ...
        # print("LDA process", X.shape)
        X = self.PCA_Process(curr_labels)
        print("PCA process", X.shape)
